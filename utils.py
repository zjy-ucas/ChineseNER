...
def result_to_json(string, tags):
    ...
    entity_start = 0
    idx = 0
    for char, tags in zip(string, tags):
        if tags[0] == "S":
            item["entities"].append({"word": char, "start": idx, "end": idx+1, "type":tags[2:]})
        elif tags[0] == "B":
            entity_name += char
            entity_start = idx
        elif tags[0] == "I":
            ...
            entity_name += char
            item["entities"].append({"word": entity_name, "start": entity_start, "end": idx + 1, "type": tags[2:]})
            entity_name = ""
        else:
            entity_name = ""
            entity_start = idx
